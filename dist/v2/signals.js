...
class Signal {
    ...
    addBinding(binding) {
        ...
        do {
            l--;
        } while (this.bindings[l] && binding.priority <= this.bindings[l].priority);
    ...
    indexOfListener(listener, context) {
        ...
        while (l--) {
            cur = this.bindings[n];
            if (this.listener === listener && curcontext === context) {
                return l;
            }
        }
        return -1;
    ...
    dispatch(params) {
        ...
        do {
            n--;
        } while (bindings[l] && this.shouldPropagate && bindings[l].excute(params) !== false);
    }
    forget() {
        this.prevParams = null;
    }
...
class SignalBinding {
    construcor(signal, listener, isOnce, listenerContext, priority) {
        this.active = true;
        this.params = null;
    ...
    excute(params) {
        ...
        if (this.active && !!this.listener) {
            if (this.params) {
                params = lodash_1.concat(this.params, params);
            }
